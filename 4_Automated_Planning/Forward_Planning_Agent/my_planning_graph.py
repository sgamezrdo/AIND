...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        if len([ef for ef in effects_B if (~ef) in effects_A]) > 0:
            return True
        else:
            return False
    ...
    def _interference(self, actionA, actionB):
        ...
        precon_A = self.parents[actionA]
        precon_B = self.parents[actionB]
        #B effects that negate A precon
        beff_neg = [ef for ef in effects_B if (~ef) in precon_A]
        #A effects that negate B precon
        aeff_neg = [ef for ef in effects_A if (~ef) in precon_B]
        
        if len(beff_neg + aeff_neg) > 0:
            return True
        else:
            return False
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        # TODO: implement this function
        #raise NotImplementedError
        #Extract preconditions from the parent layer
        precon_A = self.parents[actionA]
        precon_B = self.parents[actionB]
        #print('#####')
        #print(precon_A)
        #B effects that negate A precon
        #self.parent_layer._mutexes[act_A]
        a_mutex = [prec_a for prec_a in precon_A for prec_b in precon_B if self.parent_layer.is_mutex(prec_b, prec_a)]
        #A effects that negate B precon
        b_mutex = [prec_b for prec_b in precon_B for prec_a in precon_A if self.parent_layer.is_mutex(prec_b, prec_a)]
        a_nmutex, b_nmutex = len(a_mutex), len(b_mutex)

        if (a_nmutex + b_nmutex) > 0:
            return True
        else:
            return False
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        # TODO: implement this function
        #raise NotImplementedError
        #all ways to achieve both literals
        actions_A = self.parents[literalA]
        actions_B = self.parents[literalB]
        #for every action in A
        ##for every action in B
        ###return the action A whose mutexes contains the action B
        #all combinations
        all_combs = len([act_A for act_A in actions_A for act_B in actions_B])
        mutexes_A = [act_A for act_A in actions_A for act_B in actions_B if self.parent_layer.is_mutex(act_A, act_B)]
        mutexes_B = [act_B for act_B in actions_B for act_A in actions_A if self.parent_layer.is_mutex(act_A, act_B)]

        if (len(mutexes_A) == all_combs) and (len(mutexes_B) == all_combs):
            return True
        else:
            return False
...
class PlanningGraph:
    ...
    def level_cost(self, goal):
        """
        Auxiliar function defined as proposed in heuristics.md.
        It returns in what layer the goal is first found.
        """
        self.fill()
        for i, litlayer in enumerate(self.literal_layers):
            if goal in litlayer:
                return i
